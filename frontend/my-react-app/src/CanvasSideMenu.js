...
import './PipelineSideMenu.css'; // Import the CSS file for styling
import { useDarkMode } from './DarkModeContext'; // Import the DarkModeContext


const PrototypeSideMenu = ({ handleCanvasSheetSelection, sheetTitles, onSheetSelect, sheetData, xlsxTitle, dataStore, handleSelectedSheet, schemaConfigSelected }) => {
...
//depcerecated
  ...
  const handleSheetSelect = (index, sheetTitle) => {
   ...
   const handleCanvasSheetSelected = (index, structTitle) =>{
     console.log(structTitle)
     setSelectedSheet(structTitle);
     console.log(selectedSheet)
     handleCanvasSheetSelection(structTitle)
   }
 ...
 return (
   <div className='pipelineInstance'>
     <div
       className={`pipeline-side-menu2 ${isResizing ? 'resizing' : ''} ${
       ...
       <div className='sidemenu-title'> </div>
        {dataStore.canvases ? dataStore.canvases.map((title, index) => (
          <li
             ...
             onClick={() => handleCanvasSheetSelected(index, title)}
           >
             {title}
           </li>
